...
layui.config({}).extend({}).define(['laytpl', 'laypage', 'layer', 'form'], function(exports) {
	...
	var $ = layui.$,
		...
		table = {
            ...
            //是否存在节点
            queryNode: function (tableId, obj) {
                var that = table.getClass(tableId);
                if (!that || !obj) return;
                var id = obj[that.config.idField];
                var maps = table.getDataMap(tableId);
            ...
            //展开下级列表
            treeExpandRows: function (that, o) {
			...
			if (e.children && e.children.length > 0) {
				...
				for (var i = 0; i < temList.length; i++) {
					...
					if (o[table.config.cols.isOpen]) { //打开状态的，关闭
                        ...
                        temo = that.layBody.find('tr[data-index="' + n[table.config.indexName] + '"]');
	...
	Class.prototype.syncCheckAll = function() {
		...
		list.forEach(function(t) {
			if (!t[table.config.cols.cheDisabled]) {
				if (t[table.config.cols.isCheckName]) {
                    ...
                    temis = false;
                    checkAllElem = that.layBody.find('tr[data-index=' + t[table.config.indexName] + ']').find('input[name="' +
						TABLE_CHECKBOX_ID + '"]');
        ...
        if (checkNum < (list.length - options.cheDisabledNum)) {
            checkAllElem = that.layHeader.find('input[name="' + TABLE_CHECKBOX_ID + '"]');
			checkAllElem.prop('checked', false);
		}
    ...
    Class.prototype.eventsinit = function () {
        ...
        that.elem.on('click', 'i.layui-tree-head', function () {
            ...
            var o = datas[index];
            //typeof options.onExpandRow === 'function' && options.onExpandRow(that, index, o);
            that.treeNodeOpen(o, !o[table.config.cols.isOpen]);
            that.resize();
        });
        //复选框选择
