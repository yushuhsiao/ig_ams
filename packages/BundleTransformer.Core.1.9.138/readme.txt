   ...
   DESCRIPTION
   ===========
   Bundle Transformer - a modular extension for System.Web.Optimization (also known
   as the Microsoft ASP.NET Web Optimization Framework). Classes `StyleTransformer`
   and `ScriptTransformer`, included in the core of Bundle Transformer and
   implement interface `IBundleTransform`. They are intended to replace the
   ...
   assets, support automatic transformation of relative paths to absolute in
   CSS-code (by using `UrlRewritingCssPostProcessor`), etc. These classes do not
   ...
   ====================
   For debugging HTTP-handlers to use a configuration settings from bundles need to
   add in the `RegisterBundles` method of `App_Start/BundleConfig.cs` file the
   following code:

   BundleResolver.Current = new CustomBundleResolver();

   In order to these settings can be applied to CSS- and JS-assets need to register
   the debugging HTTP-handlers `CssAssetHandler` and `JsAssetHandler` in Web.config
   file.
