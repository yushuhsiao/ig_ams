...
-- =============================================
-- Description: 存入 Major
-- Update date: 2016-12-07
-- =============================================
CREATE PROCEDURE dbo.usp_PushMajorJackpot
...
BEGIN TRY
    BEGIN TRANSACTION;
        ...
        IF @Jackpot IS NULL
        BEGIN
            SET @Jackpot = @Amount + (SELECT Base FROM dbo.JackpotConfig WHERE JackpotType = 'MAJOR');

            INSERT INTO dbo.Jackpot (PlayerId, GameId, JackpotType, Amount)
            VALUES (0, 0, 'MAJOR', @Jackpot);
        END
        ELSE
        BEGIN
        ...
        BEGIN
            ...
            VALUES (@PlayerId, @GameId, @SerialNumber, 'MAJOR', @Jackpot, @Amount, @Date);
        END;
