...
-- =============================================
-- Description: 提取 Grand，提取 Jackpot * @Ratio / 100，剩餘的 Jackpot 若小於打底時，補到打底值
-- Update date: 2016-12-30
-- =============================================
CREATE PROCEDURE dbo.usp_PullGrandJackpot
...
BEGIN TRY
    BEGIN TRANSACTION;
        ...
        -- 取得目標值跟打底
        SELECT @Goal = Goal, @Base = Base FROM dbo.JackpotConfig WHERE JackpotType = 'GRAND';

        -- 取得目前的 Jackpot
        SET @Jackpot = (SELECT Amount FROM dbo.Jackpot WITH (UPDLOCK) WHERE JackpotType = 'GRAND');
        ...
        VALUES (@PlayerId, @GameId, @SerialNumber, 'GRAND', @Jackpot, @Base, @Ratio, @WinAmount, @BaseAmount, @FillAmount, @Date);
        ...
        VALUES (@SerialNumber, @PlayerId, @GameId, 'PULLJP', '', 'GRAND', 0, @WinAmount, 0, @WinAmount, @Jackpot, @Base, @Ratio, @BaseAmount, @FillAmount, 0, 0, 0, 0, @Date);
