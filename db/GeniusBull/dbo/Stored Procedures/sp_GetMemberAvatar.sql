...
declare
	...
	begin
		...
		where b.OwnerId = @PlayerId and isnull(c.[State], 0) = 0 order by a.LastLoginTime asc
		if @AvatarId is null
			RAISERROR (2147483647, 18, 255, 'unable to alloc AvatarId, AvatarId is null')

		select @Nickname = Nickname, @LastLoginIp = LastLoginIp from dbo.Member with(nolock) where Id = @PlayerId
		update dbo.Member set Nickname = isnull(@Nickname, Nickname), LastLoginIp = @LastLoginIp, LastLoginTime = getdate(), AccessToken=@AccessToken
		where Id = @AvatarId
