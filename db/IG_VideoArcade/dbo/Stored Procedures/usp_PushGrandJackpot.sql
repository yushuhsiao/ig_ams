...
-- =============================================
-- Description: 存入 Grand
-- Update date: 2016-12-07
-- =============================================
CREATE PROCEDURE dbo.usp_PushGrandJackpot
...
BEGIN TRY
    BEGIN TRANSACTION;
        ...
        IF @Jackpot IS NULL
        BEGIN
            SET @Jackpot = @Amount + (SELECT Base FROM dbo.JackpotConfig WHERE JackpotType = 'GRAND');

            INSERT INTO dbo.Jackpot (PlayerId, GameId, JackpotType, Amount)
            VALUES (0, 0, 'GRAND', @Jackpot);
        END
        ELSE
        BEGIN
        ...
        BEGIN
            ...
            VALUES (@PlayerId, @GameId, @SerialNumber, 'GRAND', @Jackpot, @Amount, @Date);
        END;
