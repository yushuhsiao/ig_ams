...
namespace System.Reflection
{
        ...
        public static IEnumerable<T> GetInstances(object target, string name)
        {
            ...
            lock (instances)
            {
                if (instances.TryGetValue(targetType, out attrs) == false)
                {
                    ...
                    foreach (MethodInfo m in targetType.GetMethods(_TypeExtensions._BindingFlags))
                    {
    ...
    public abstract class ObjectInvokeAttribute : Attribute
    {
        ...
        public object Invoke(IServiceProvider provider, object target, Dictionary<string, dynamic> datas,
            Type FromServices = null,
            Type FromBody = null, object msg = null, string msg_body = null)
        {
            ...
            for (int i = 0; i < pp.Length; i++)
            {
                try
                {
                    ...
                    {
                        args[i] = provider.GetService(p.ParameterType);
                        continue;
                    }
                    if ((FromBody != null) && p.GetCustomAttribute(FromBody) != null)
                    {
                        if (msgType == null) continue;
                        if (p.ParameterType.IsAssignableFrom(msgType))
                            args[i] = msg;
