...
namespace Jsbeautifier
{
    public class Beautifier
    {
        ...
        {
            this.Opts = opts ?? new BeautifierOptions();
            this.BlankState();
        }
        ...
        public static string ProcessText(string s, BeautifierOptions opts = null, BeautifierFlags flags = null)
        {
            Beautifier obj = new Beautifier(opts);
            obj.Flags = obj.Flags ?? flags;
            return obj.Beautify(s, opts);
        }
        ...
        public string Beautify(string s, BeautifierOptions opts = null)
        {
            ...
            while (true)
            {
                ...
                string tokenText = token[0];
                string tokenType = token[1];
                if (tokenType == "TK_EOF")
                    break;
            ...
            string sweetCode = this.PreindentString + regex.Replace(string.Concat(this.Output.ToArray()), "", 1);
            return sweetCode;
        }
        ...
        private string[] GetNextToken()
        {
            ...
            if (keepWhitespace)
            {
                ...
                while (Enumerable.Contains(this.Whitespace, c))
                {
                    if (c == '\n')
                    {
                        this.TrimOutput();
            ...
            if (Enumerable.Contains(this.Wordchar, c))
            {
                if (this.ParserPos < this.Input.Length)
                {
                    cc = c.ToString();
                    while (Enumerable.Contains(this.Wordchar,this.Input[this.ParserPos]))
                    {
                ...
                if (this.ParserPos != this.Input.Length && Enumerable.Contains("+-",this.Input[this.ParserPos]) && Regex.IsMatch(cc, "^[0-9]+[Ee]$"))
                {
                    char sign = this.Input[this.ParserPos];
                    this.ParserPos++;
                    var t = this.GetNextToken();
                    cc += sign + t[0];
                    return new string[] { cc, "TK_WORD" };
                }

                if (cc == "in") // in is an operator, need to hack
                    return new string[] { cc, "TK_OPERATOR" };
            ...
            if (Enumerable.Contains("([", c))
                return new string[] { c.ToString(), "TK_START_EXPR" };

            if (Enumerable.Contains(")]", c))
                return new string[] { c.ToString(), "TK_END_EXPR" };

            if (c == '{')
                return new string[] { c.ToString(), "TK_START_BLOCK" };

            if (c == '}')
                return new string[] { c.ToString(), "TK_END_BLOCK" };

            if (c == ';')
                return new string[] { c.ToString(), "TK_SEMICOLON" };
            ...
            if (c == '/')
            {
                ...
                if (this.Input[this.ParserPos] == '*') // peek /* .. */ comment
                {
                    ...
                    this.ParserPos += 2;
                    return new string[] { "/*" + comment + "*/", commentMode };
                }

                if (this.Input[this.ParserPos] == '/') // peek // comment
                {
                    comment = c.ToString();
                    while (!Enumerable.Contains(("\r\n"),this.Input[this.ParserPos]))
                    {
            ...
            if (c == '\'' || c == '"' ||
                ...
                (Enumerable.Contains((new[] { "TK_COMMENT", "TK_START_EXPR", "TK_START_BLOCK", "TK_END_BLOCK", "TK_OPERATOR", "TK_EQUALS", "TK_EOF", "TK_SEMICOLON", "TK_COMMA" }),this.LastType)))))
            {
                char sep = c;
                bool esc = false;
                int esc1 = 0;
                ...
                if (sep == '/')
                {
                    ...
                    while (this.ParserPos < this.Input.Length && Enumerable.Contains(this.Wordchar,this.Input[this.ParserPos]))
                    {
                        resultingString += this.Input[this.ParserPos];
                        this.ParserPos += 1;
                    }
            ...
            if (c == '#')
            {
                ...
                if (this.ParserPos < this.Input.Length && Enumerable.Contains(this.Digits,this.Input[this.ParserPos]))
                {
                    while (true)
                    {
                        c = this.Input[this.ParserPos];
            ...
            if (c == '<' && this.Input.Substring(this.ParserPos - 1, Math.Min(4, this.Input.Length - this.ParserPos + 1)) == "<!--")
            {
                ...
                return new string[] { ss, "TK_COMMENT" };
            }
            ...
            if (c == '.')
                return new string[] { ".", "TK_DOT" };

            if (Enumerable.Contains(this.Punct, c.ToString()))
            {
                string ss = c.ToString();
                while (this.ParserPos < this.Input.Length && Enumerable.Contains(this.Punct, ss + this.Input[this.ParserPos]))
                {
                    ss += this.Input[this.ParserPos];
                    this.ParserPos += 1;
                    if (this.ParserPos >= this.Input.Length)
                ...
                if (ss == "=")
                    return new string[] { "=", "TK_EQUALS" };

                if (ss == ",")
                    return new string[] { ",", "TK_COMMA" };

                return new string[] { ss, "TK_OPERATOR" };
            }

            return new string[] { c.ToString(), "TK_UNKNOWN" };
        }
        ...
        private void HandleStartExpr(string  tokenText)
        {
            if (tokenText == "[")
            {
                ...
                {
                    if (Enumerable.Contains(this.LineStarters,this.LastText))
                        this.Append(" ");
            ...
            else if (Enumerable.Contains(this.LineStarters,this.LastText) || this.LastText == "catch")
                this.Append(" ");
        ...
        private void HandleWord(string tokenText)
        {
            ...
            if (Enumerable.Contains(this.LineStarters, tokenText))
            {
                if (this.LastText == "else")
                    prefix = "SPACE";
                else
            ...
            else if (prefix == "NEWLINE")
            {
                ...
                else if (Enumerable.Contains(this.LineStarters, tokenText) && this.LastText != ")")
                {
                    this.Flags.VarLine = false;
                    this.Flags.VarLineReindented = false;
                    this.AppendNewline();
        ...
        private void HandleOperator(string tokenText)
        {
            ...
            if ((tokenText == "++" || tokenText == "--" || tokenText == "!") || (tokenText == "+" || tokenText == "-") &&
                ...
                (Enumerable.Contains(this.LineStarters,this.LastText) || this.LastText == ",")))
            {
                spaceBefore = false;
                spaceAfter = false;
                ...
                if (this.LastText == "TK_WORD" && Enumerable.Contains(this.LineStarters,this.LastText))
                    spaceBefore = true;
