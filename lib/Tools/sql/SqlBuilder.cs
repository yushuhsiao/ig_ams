...
namespace System.Data
{
    ...
    public class SqlBuilder : DynamicObject, IDictionary<string, object>
    {
        ...
        public struct err : _op
        {
            ...
            void _op.Build(SqlBuilder src, StringBuilder s)
            {
        ...
        public interface _op { void Build(SqlBuilder src, StringBuilder s); }
                ...
                for (int _index = 0, _count = src.items.Count; _index < _count; _index++)
                {
                    ...
                    if (this._value)
                    {
                        ...
                        if (n.nvarchar)
                            ...
                            format = System.Data.SqlClient.SqlCmd.varchar;
                        else
                            format = n.format;
        ...
        public string _insert(out string pattern) { return __op(SqlBuilder.op.insert, out pattern); }
        ...
        public string _where(out string pattern) { return __op(SqlBuilder.op.where, out pattern); }
        public string _where(bool pattern_only = false) { return __op(op.where, pattern_only); }

        public string _fields(out string pattern) { return __op(SqlBuilder.op.Fields, out pattern); }
        public string _fields(bool pattern_only = false) { return __op(op.Fields, pattern_only); }

        public string _values(out string pattern) { return __op(SqlBuilder.op.Values, out pattern); }
        public string _values(bool pattern_only = false) { return __op(op.Values, pattern_only); }

        public string _err(SqlBuilder.err err, out string pattern) { return __op(err, out pattern); }
        public string _err(SqlBuilder.err err, bool pattern_only = false) { return __op(err, pattern_only); }

        public string _update_set(out string pattern) { return __op(SqlBuilder.op.update_set, out pattern); }
