...
namespace System.Data
{
    ...
    public class SqlSchemaTable : Dictionary<string, Type>
    {
        ...
        {
            foreach (FieldInfo f in obj.GetType().GetFields(_TypeExtensions.BindingFlags0))
                if (this.HasField(f.Name))
                    yield return f;
            foreach (PropertyInfo p in obj.GetType().GetProperties(_TypeExtensions.BindingFlags0))
