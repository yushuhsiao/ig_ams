...
namespace ams.Data
{
    ...
    public partial class IG01PlatformInfo : PlatformInfo<IG01PlatformInfo, IG01MemberPlatformData>
    {
        ...
        {
            string TableName = TableName<IG01MemberPlatformData>.Value;
            ...
            for (int i = 0; ; i++)
            {
                ...
                IG01MemberPlatformData m1 = userdb.ToObject(() => new IG01MemberPlatformData() { Member = member }, true,
...
select * from {TableName} nolock {sql0_where}");
                try
                {
                    ...
                    foreach (Action commit in gamedb.BeginTran())
                    {
                        ...
                        try { m2 = gamedb.ToObject<GeniusBull.Member>(api_CreateUser(m1, member)); }
                        catch (SqlException ex) when (ex.IsDuplicateKey()) { break; }
                        IG01MemberPlatformData _chk = userdb.ToObject<IG01MemberPlatformData>($"select * from {TableName<IG01MemberPlatformData>.Value} where PlatformID={this.ID} and destID={m2.Id}");
                        if (_chk != null)
                            break;
                        commit();
                        //sql0["u", "Active"] = MemberPlatformActiveState.Active;
                        //sql0["u", "destID"] = m2.Id;
                        return userdb.ToObject<IG01MemberPlatformData>(true, $@"update {TableName} set Active={(int)MemberPlatformActiveState.Active}, destID={m2.Id} {sql0._where()}
select * from {TableName} nolock {sql0_where}");
                    }
                    if (i > 100)
        ...
        public override ForwardGameArguments ForwardGame(_ApiController controller, ForwardGameArguments _args)
        {
     ...
     update {TableName<MemberDetailData>.Value} set PhotoRegistered=getdate() where ID={member.ID} and PhotoRegistered is null
        ...
        public override bool Deposit(MemberData member, decimal amount, out decimal balance, bool force)
        {
            for (int retry = 0; ; retry++)
            {
                IG01MemberPlatformData m1 = this.GetMember(member, true);
                if (m1 == null) throw new _Exception(Status.PlatformApiFailed);
                try { return api_UpdateBalance(m1, member, amount, out balance, true); }
                catch (_Exception ex) when (ex.StatusCode == Status.PlatformUserNotExist) { _retry(retry, member, m1); }
            }
        }
        ...
        public IG01MemberPlatformData GetMemberByDestID(int destID, bool getMemberData = false, bool getMemberData_err = false)
        {
            ...
            foreach (CorpInfo corp1 in corps)
            {
                ...
                SqlCmd userDB1;
                using (_HttpContext.GetSqlCmd(out userDB1, null, corp1.DB_User01R))
                {
                    ...
                    if (m2 != null)
                    {
                        if (getMemberData)
                        {
                            foreach (CorpInfo corp2 in corps)
                            {
                                ...
                                SqlCmd userDB2;
                                using (_HttpContext.GetSqlCmd(out userDB2, userDB1, corp1.DB_User01R))
                                {
        ...
        public GeniusBull.Game GetGame(int gameID) => this.GetGames().Find((g) => g.Id == gameID);

        public List<GeniusBull.Game> GetGames()
        {
            RedisVer<List<GeniusBull.Game>> _games;
            lock (_games_sync)
            {
...
select Id, Account, AccessToken from {TableName<GeniusBull.Member>.Value} nolock where Id={m1.destID}";
        ...
        string api_CreateUser(IG01MemberPlatformData p_member, MemberData member, string accessToken = null)
        {
            SqlBuilder sql0 = new SqlBuilder();
            sql0[" ", "ParentId     "] = 0;
...
select Id, Account, AccessToken from {TableName<GeniusBull.Member>.Value} nolock {sql0._where()}";
        ...
        //            foreach (Action commit in gamedb.BeginTran())
        //            {
        //                try { return gamedb.ToObject<GeniusBull.Member>(sql1); }
        //                catch (SqlException ex) when (ex.IsDuplicateKey()) { }
        //            }
        //            return null; ;
        //        }

        bool api_UpdateBalance(IG01MemberPlatformData m1, MemberData member, decimal amount, out decimal balance, bool force)
        {
            balance = 0;
...
select Balance from {GeniusBull.Member.TableName} nolock where Id={m1.destID}";
            ...
            foreach (Action commit in sqlcmd.BeginTran())
            {
                ...
                if (b.HasValue)
                {
                    if ((force == false) && (b.Value < 0)) return false;
                    balance = b.Value;
                    commit();
                    return true;
                }
