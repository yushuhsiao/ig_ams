...
group by GameId, Eprob, Selected";
            return gamedb.ToList<EprobTable.Group>(sql) ?? _null<EprobTable.Group>.list;
        }
...
order by a.Symbol";
            ...
            List<EprobTable> list = gamedb.ToList<EprobTable>(sql) ?? _null<EprobTable>.list;
     ...
     update {TableName<EprobTable>.Value}{sql._update_set()}{sql._where()}
        ...
        public List<JackpotConfig> GetJackpotConfigList(_empty args)
        {
            ...
            return jackpot_p.GameDB().ToList<JackpotConfig>($"select * from {TableName<JackpotConfig>.Value} nolock") ?? _null<JackpotConfig>.list;
        }
    ...
    public class GeniusBullUserApiController : _ApiController
    {
        ...
        public List<MemberBlacklist> GetBlackList(_empty args)
        {
            ...
            List<MemberBlacklist> ret = platform.GameDB().ToList<MemberBlacklist>($"select * from {TableName<MemberBlacklist>.Value} nolock where MemberId={m2.destID}") ?? _null<MemberBlacklist>.list;
