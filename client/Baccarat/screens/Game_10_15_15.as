...
private var temp_game: String;		
		...
		private var myTimer: Timer = new Timer(timerRefreshRate);
		//game
		...
		private function onUpdate(e: EnterFrameEvent): void {
			...
			if(connect_.win_f){
				...
				if (temp_game != connect_.bet_details.Game) {
					//reset();
				}
		...
		private function win_event(): void {
			...
			var count_win:Number = 0;
					...
					//trace (connect_.win_details.win, bet_name[i]);
						if(connect_.win_details.win == bet_name[i]){
							temp_win[i] = 1;
							count_win += 1;
						}
						if(temp_win[i] == 0){
