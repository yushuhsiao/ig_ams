...
private var bet_name: Array = new Array("Any-Pair","Big","Player-Pair","Player","Tie","Super 6","Banker","Banker-Pair","Small","Perfect-Pair");
...
private var temp_game: String;		
		...
		private var cardx_holder: Array = new Array(-110, -63, -23, 23, 63, 110);
		...
		private function onAddedToStage(event: Event): void {
			...
			stage.addEventListener(EnterFrameEvent.ENTER_FRAME, onUpdate);
		}
		...
		private function onUpdate(e: EnterFrameEvent): void {
			...
			connect_.account_f = false;
				...
				TweenLite.delayedCall(2,win_event);
		...
		private function panel_timer(): void {
			...
			timer_text = new TextField(100, 50, "", "BERNHC", 50, 0xFFFFFF, true);
		...
		private function panel_selection(): void {
			...
			total_balance_text.x = cont_cash.x - 55;
			total_balance_text.y = cont_cash.y + 12;
			total_balance_text.text = total_balance.toFixed(1).toString();
		...
		private function onTouch(e: TouchEvent): void {
			...
			if (e.getTouch(cont_btn_bet, TouchPhase.BEGAN)) {
				if (connect_.status_ == "Connected" && con_timer != 0) bet_event();
			}
		...
		private function auto_event(): void {
			...
			for (var i: Number = 0; i < gameSettings.bet_tag.length; i++) {
				...
				for (m = 0; m < bets_[i]; m++) {
					//TweenLite.to(o.bet_[i][m],.3,{alpha:0});
					this.removeChild(o.bet_[i][m]);
				}
				for (m = 0; m < bets_Area[i]; m++) {
					//TweenLite.to(p.bet_[i][m],.3,{alpha:0});					
		...
		private function win_event(): void {
					var count:Number = 0;
			var count_win:Number = 0;
			
			
					for (var i: Number = 0; i < bet_name.length; i++) {
					//trace (connect_.win_details.win, bet_name[i]);
							for (var l: Number = 0; l <connect_.win_details.win.length; l++) {
						if(connect_.win_details.win[l] == bet_name[i]){
							temp_win[i] = 1;
							//count_win += 1;
						}
					}
				}
				for ( i= 0; i < bet_name.length; i++) {
						if(temp_win[i] == 0){
						//count += bets_Area[i];
					...
					TweenLite.delayedCall(.5,function(){win_status = true;for ( i= 0; i < bet_name.length; i++) {
							if(temp_win[i]==1){
								chip_bet_check(i, Math.ceil(connect_.bet_details.bet[i] * bet_value[i]));
									total_balance += bets[i] * bet_value[i];
								temp_bal  = total_balance;
			total_balance_text.text = total_balance.toFixed(1).toString();
								}
					...
					TweenLite.delayedCall(2.5,function(){	for ( i= 0; i < bet_name.length; i++) {
							if(temp_win[i]==1){for (m = 0; m < bets_Area[i]; m++) {
					TweenLite.to(p.bet_[i][m],1,{x:20,y:420,alpha:0,delay: Math.random() * .6});
					}}}});
					
					TweenLite.delayedCall(4,function(){	for (i= 0; i < bet_name.length; i++) {
						for (m = bets_[i] -1; m >=0; m--) {
					TweenLite.to(o.bet_[i][m],.5,{alpha:0});
					}
					}
					TweenLite.to(cont_score,.5,{alpha:0,onComplete:reset});
					con = false;});
					//TweenLite.delayedCall(4.5,function reset());
